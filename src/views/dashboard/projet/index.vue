...
export default {
  ...
  mounted() {
    // Effectuer une requête HTTP GET pour récupérer les données des projets
    axios.get('https://ssise-cosit.com/api-ssise/projet/getAllProjet')
      .then(response => {
        // Traiter les données reçues dans la réponse
...
<template>
  <Layout>
    ...
    <BRow>
      <BCol xxl="3" sm="6" class="project-card" v-for="(item, index) of  projects " :key="index">
